...
import { useBookDayUseSlot } from "../../../ApiHooks/useDayUseRoomHook";
...
const PaymentOptionModal = ({ isOpen, onClose, room, selectedDate, checkInTime, checkOutTime }) => {
  ...
  const { mutateAsync: bookSlot, isLoading: booking } = useBookDayUseSlot(); // ⬅️ booking mutation

  // local UI submit state (covers both enquiry + booking)
  ...
  const handleBookingSubmit = async () => {
    ...
    const cleanDate = format(new Date(selectedDate), 'yyyy-MM-dd');

    // 1) Enquiry payload (your existing flow)
    const enquiryPayload = {
      ...
      user: {
        firstName: `${firstName} ${lastName}`.trim(), // small fix: proper spacing
        // lastName, // if backend expects separate fields, send too
        email,
        phone,
      },
      source: 'website',
    ...
    // 2) Booking (availability decrement) payload
    const bookPayload = {
      hotelCode: room?.hotel?.hotelCode,
    ...
    // Guard: ensure we have booking fields
    if (!bookPayload.hotelCode || !bookPayload.roomName) {
      toast.error('Hotel/Room details missing for booking');
      return;
    }
    ...
    try {
      ...
      // Step A: Create enquiry (server record / lead)
      ...
      // Step B: Decrement availability atomically
      await bookSlot(bookPayload);

    //   toast.success('Booking submitted & availability updated!');
      onClose();
    } catch (err) {
  ...
  const isBusy = submitting || booking || enquiry.isLoading;
        ...
        >
          ...
          <div className="flex justify-between items-center gap-4 p-6 bg-gradient-to-r from-gray-50 to-gray-100 rounded-b-3xl">
            ...
            <button
              ...
              className="flex-1 px-6 py-3 text-sm font-semibold text-white bg-primary-green rounded-xl shadow-lg hover:bg-primary-green/90 transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-primary-green disabled:opacity-60 disabled:cursor-not-allowed"
              disabled={isBusy}
            >
