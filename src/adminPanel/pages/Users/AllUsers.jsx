...
import AgentCorporateManager from './AgentCorporateManager';

const AllUsers = () => {
  const { allUsers, isUsersLoading, usersError } = useAdminContext();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState('users'); // 'users' or 'agents'
    ...
    <div className="flex flex-col gap-10 mx-auto p-6 pt-28 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-extrabold">
          {activeTab === 'users' ? 'All Users' : 'Agent & Corporate Manager'}
        </h1>
        {activeTab === 'users' && (
          <button
      ...
      {/* Tab Content */}
      {activeTab === 'users' ? renderUsersTable() : <AgentCorporateManager />}
    </div>
  );
};
